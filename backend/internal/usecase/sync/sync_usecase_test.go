...
func validInput() syncUsecase.SyncInput {
	return syncUsecase.SyncInput{RequestID: "req-1", DeviceID: "device-1", Changes: []syncUsecase.SyncChange{{EntityType: "TRANSACTION", EntityID: "00000000-0000-0000-0000-000000000001", Ciphertext: []byte("cipher"), Nonce: []byte("nonce"), Version: 1, UpdatedAt: time.Now().UTC()}}}
}
...
func TestSyncUsecase_MapsConflictsAndServerChanges(t *testing.T) {
	rec := &entity.SyncRecord{UserID: "user", EntityType: entity.EntityTypeTransaction, EntityID: "id", Ciphertext: []byte("c"), Nonce: []byte("n"), Version: 2, UpdatedAt: time.Now().UTC(), ServerRevision: 12}
